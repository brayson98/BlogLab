        ...
        public async Task<ActionResult<int>> Delete(int photoId)
        {
            ...
            if (foundPhoto != null)
            {
                if (foundPhoto.ApplicationUserId == applicationUserId)
                {
                    ...
                    var usedInBlogs = blogs.Any(b => b.PhotoId == photoId);

                    if (usedInBlogs) return BadRequest("This Photo is currently being used in a live blog.");
